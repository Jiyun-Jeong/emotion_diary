...
const DiaryEditor= ({ isEdit, originData }) => {
  ...
  const handleSubmit = () => {
    ...
    if(window.confirm(isEdit? "일기를 수정하시겠습니까?" : "작성을 완료하시겠습니까?")) {
      if(isEdit) {
        onCreate(date, content, emotion);
      } else{
        onEdit(originData.id, data, content, emotion);
      }
    }
      ...
      <div>
        ...
        <section>
          ...
          <div className="input_box text_wrapper">
            <textarea placeholder="오늘은 어땠나요?" title="일기 내용" ref={contentRef} value={content} onChange={(e)=>setContent(e.target.value)} />
          </div>
        </section>
        <section>
